...
import {DeleteMailsModalWindow} from "../ui/deleteMailsModalWindow.tsx";
...
export const DeleteMailsButton = (props: ManageMailsButtonType) => {
    ...
    return (
        <>
            <button disabled={props.isDisabled} className={styles.buttonWrapper} onClick={toggle}>
                Удалить
            </button>
            <DeleteMailsModalWindow
                windowName={"удаление писем"}
                isShown={isShown} hide={toggle}
                onPerform={() => {mailStore.deleteMails(folderStore.getCurrentFolderId())}}
            />
        </>
    )
}